    ...
    unpackPhase = ''
      ...
      for srcFile in $srcs; do
        ...
        if [ -d $srcFile ]; then
          ...
          chmod 600 server-tmp/server-icon.png server-tmp/variables.txt server-tmp/server.properties server-tmp/start.*
          rm -fv server-tmp/server-icon.png server-tmp/variables.txt server-tmp/server.properties server-tmp/start.*
